...
func _ready() -> void: 
	...
	if tail:
		...
		if !tail.texture:
			...
			printerr("(BasicNote (lane: %s)): could not find sustain animation (%s). Using fallback texture" % [lane, tail_animation])
	...
	if end:
		...
		if end.texture:
			...
			printerr("(BasicNote (lane: %s)): Could not find end animation (%s)." % [lane, tail_animation])
